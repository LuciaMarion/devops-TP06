...
def health():
    try:
        ...
        db_status = f"error: {e}"
    return jsonify({"status": "ok", "db": db_status, "time": datetime.datetime.utcnow().isoformat()})
...
def create_note():
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("INSERT INTO notes (title, content) VALUES (%s, %s) RETURNING id", (data["title"], data.get("content", "")))
